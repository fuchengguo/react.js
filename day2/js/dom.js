...
((global, document, Object, vnodeFlag, globalState, createElement, render, wrapClassProxy) => {
  ...
  createElement = (element, props = {}, ...children) => ({
    ...
    P: props[vnodeFlag] // If the props argument is a renderable VNode,
      ? {
        C: [].concat(props, ...children)
      } // ... prepend it to the children
   ...
   * @param {HTLDomElement}
   */
  global.R = render = (vnodes, // 1. The vnode tree to render
    ...
    (vnodes.map
      ? vnodes
      : [vnodes]).map( // Cast `vnodes` to array if nor already
    ...
    _path = _npath + '.' + index, // a. The state path of this vnode
        _path_state = globalState[_path] || [
      {}, vnode.E
    ], // b. Get the state record for this path
      ...
      (_new_dom = _child // If we have a previous child we first check if
        ? (_child.E != vnode.E && _child.data != vnode) // the VNode element or the text are the same
          ...
          ? dom.replaceChild( // - If not, we replace the old element with the
              _new_dom, //   new one.
              _child) && _new_dom //   ... and we make sure we return the new DOM

          : _child // - If it's the same, we keep the old child

        : dom.appendChild( // If we don't have a previous child, just append
            _new_dom)).E = vnode.E; // We keep the vnode element to the .E property in
      ...
      vnode.trim
        ...
        : Object.keys(vnode.P).map( // - Element nodes have properties
          ...
          ? // The 'style' property is an object and must be
          // applied recursively.
          Object.assign(_new_dom[key], // '[key]' is shorter than '.style'
              vnode.P[key])
   ...
   */
  wrapClassProxy = (factoryFn) => new Proxy( // We are creating a proxy object for every tag in
  // order to be able to customize the class name
  // via a shorthand call.
  factoryFn, {
    get: (targetFn, className, _instance) => wrapClassProxy((...args) => ((_instance = targetFn(...args)) // We first create the Virtual DOM instance by
   ...
   */
  global.H = new Proxy(createElement, {
    get: (targetFn, tagName) => targetFn[tagName] || wrapClassProxy(createElement.bind(global, tagName))
  })
